...
We also place a vague shifted negative binomial prior on the size of the 
latent entity population. 
```{r}
populationSize <- ShiftedNegBinomRV(2.0, 0.001)
tibble(prob = seq(from=0, to=4000, by=10)) %>%
  mutate(density = dnbinom(prob + 1, populationSize@size, populationSize@prob)) %>%
  ggplot(aes(x = prob, y = density)) + geom_line() + 
  labs(title = "Prior distribution on the population size", 
       x = "Population size", y = "Density")
```

Finally, we specify the partitioner. Since the data is small in this case, 
there is no need to partition the data, so we set the tree depth to 0 below.
```{r}
...
ggplot(diagnostics, aes(x=iteration, y=popSize)) + 
  geom_line() + 
  labs(title = 'Trace plot: population size', x = 'Iteration', 
       y = 'Size')
